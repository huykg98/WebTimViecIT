...
namespace JobSearchingSystem.DAL
{
    public class JobUnitOfWork : UnitOfWork
    {
        ...
        //get jobs of recruiters
        public IEnumerable<JJobItem> JobsOfRecruiters()
        {
            var jobList = (from a in this.JobRepository.Get()
                           where (a.IsPublic) && (a.StartedDate <= DateTime.Now) && (a.EndedDate >= DateTime.Now)
                           select new JJobItem()
                           {
                               JobID = a.JobID,
                               JobTitle = a.JobTitle,
        ...
        public IEnumerable<JobItem> GetJob()
        {
            try
            {
                List<JobItem> jobList = new List<JobItem>();
                foreach (Job job in this.JobRepository.Get(job => job.IsPublic == true))
                {
                    if ((DateTime.Now.CompareTo(job.EndedDate) > 0) && (job.IsPublic == true))
                    {
                        job.IsPublic = false;
