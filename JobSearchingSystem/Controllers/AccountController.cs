...
namespace JobSearchingSystem.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login([Bind(Include = "UserName, Password, RememberMe")]LoginViewModel model, string returnUrl)
        {
            ...
            {
                TempData["warningmessage"] = "oops something went wrong, please try again!";
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> Admin([Bind(Include = "UserName, Password, RememberMe")]LoginViewModel model, string returnUrl)
        {
            try { 
                var user = await UserManager.FindAsync(model.UserName, model.Password);
                if (user.UserName == "merchantA")
                {
                    TempData["warningmessage"] = "oops something went wrong, please try again!";

                }
                else if (user.UserName == "merchantB")
                {
                    TempData["warningmessage"] = "oops something went wrong, please try again!";

                }
                else if (user.UserName == "CustomerX")
                {
                    TempData["warningmessage"] = "oops something went wrong, please try again!";

                }
                else if (user.UserName == "customerY")
                {
                    TempData["warningmessage"] = "oops something went wrong, please try again!";
        ...
        public async Task<ActionResult> Register([Bind(Include = "UserName, Password, ConfirmPassword, Email, FullName, PhoneNumber, RoleName")]RegisterViewModel model, string returnUrl)
        {
            ...
            if (result.Succeeded)
            {
                ...
                {
                    AspNetUser user1 = unitOfWork.AspNetUserRepository.Get(s => s.UserName == model.UserName).FirstOrDefault();
                    user1.IsActive = false;
                    user1.note = "wait for active";
                    unitOfWork.AspNetUserRepository.Update(user1);
                    unitOfWork.Save();
                ...
                //    ad.AdministratorID = createdUser.Id;
                //    ad.IsDeleted = false;
                //    unitOfWork.AdministratorRepository.Insert(ad);
                //    unitOfWork.Save();
        ...
        public ActionResult List(string showoption)
        {
            ...
            else if ("2".Equals(showoption))
            {
                model.userList = (from u in accountUnitOfWork.AspNetUserRepository.Get()
                                  ...
                                  }).Where(s => s.RoleName == "Recruiter").AsEnumerable();
            }
            else if ("3".Equals(showoption))
            {
                model.userList = (from u in accountUnitOfWork.AspNetUserRepository.Get()
                                  select new AccUserItem
                                  {
                                  ...
                                  }).Where(s => s.RoleName == "Jobseeker").AsEnumerable();
            }
            else
            {
                model.userList = (from u in accountUnitOfWork.AspNetUserRepository.Get()
                                  select new AccUserItem
                                  {
                                      ...
                                      IsActive = u.IsActive,
                                      note = u.note
                                  }).Where(s => s.RoleName == "Recruiter" && s.IsActive==false).AsEnumerable();
            }
        ...
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public ActionResult AddNoteAccount(string UserName,string note, string showoption)
        {
            UnitOfWork unitOfWork = new UnitOfWork();
            AspNetUser user = unitOfWork.AspNetUserRepository.Get(s => s.UserName == UserName).FirstOrDefault();
