...
namespace JobSearchingSystem.Controllers
{
    ...
    public class JobPackageController : Controller
    {
        ...
        {
            bool state = packageUnitOfWork.AspNetUserRepository.Get(s => s.UserName == User.Identity.Name).FirstOrDefault().IsActive;
            if (state == true)
            {
            ...
            else
            {
                ...
                return RedirectToAction("JobPackageRequestListRecruiter");
            }
        }
        ...
        [AllowAnonymous]
        public ActionResult PaymentSuccess(int purchaseJobPackageID)
        {
            string userID = packageUnitOfWork.AspNetUserRepository.Get(s => s.UserName == User.Identity.Name).FirstOrDefault().Id;
